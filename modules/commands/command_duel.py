    ...
    if self.duel_toggle:
        ...
        if not duel_active:
            if data['args']:
                ...
                target = self.charMgr.load_char_by_name(name)
                if target and target != attacker and target['level'] > 0:
                    duel_active = True
                    ...
                    if target['id'] == '91953864':
                        self.run_command('!accept', {'sender_id': target['id']})
                else:
...
def challenge_player(self, attacker, target):
    ...
    def command_accept(self, data):
        ...
        logging.debug(data)
        if data['sender_id'] == target['id'] or data['sender_id'] == '91580306':
